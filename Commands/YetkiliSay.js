...
`)).sil(7);
        ...
        message.channel.send(embed.setDescription(`${aktifunsesyt.size} Yetkiliye ses çağrısı yapılıyor.`).setColor('RANDOM')).then(async(msg) => {
            aktifunsesyt.array().forEach(async(yunuz, index) => {
                setTimeout(async() => {
                    msg.edit(embed.setDescription(`${yunuz} Yetkilisine özelden mesaj atıldı.`))
                    yunuz.send(`Aktifsin fakat seste değilsin lütfen ses kanalına gir.\n**${message.guild.name}**`).catch(err => message.channel.send(`${yunuz} Aktifsin fakat seste değilsin lütfen ses kanalına gir.`) && msg.edit(embed.setDescription(`${yunuz} kullanıcısına özelden mesaj gönderilemediği için kanala etiketlendi.`)))
                }, index * 1500)
            })
